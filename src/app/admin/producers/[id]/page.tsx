...
export default function AdminProducerDetailPage() {
  ...
  const { data: producer, isLoading, refetch } = api.admin.getProducerDetails.useQuery({
    merchantId,
  });
  ...
  useEffect(() => {
    if (producer) {
      setIsVerified(producer.verificationStatus === "VERIFIED");
    }
  }, [producer]);
  ...
  if (!producer) {
    return (
      <div className="py-12 text-center">
        ...
        <h2 className="mt-4 text-lg font-semibold text-gray-900">Producer not found</h2>
        <p className="mt-2 text-gray-600">The producer you&apos;re looking for doesn&apos;t exist.</p>
        <Link
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-gray-900">{producer.businessName}</h1>
          <p className="text-gray-600">Producer Profile Review</p>
        </div>
        <span
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-6">
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2">
              <div>
                ...
                <p className="mt-1 text-gray-900">{producer.businessName}</p>
              </div>
              <div>
                <label className="text-sm font-medium text-gray-500">Registration Number</label>
                <p className="mt-1 text-gray-900">{producer.businessRegistrationNumber}</p>
              </div>
              <div>
                <label className="text-sm font-medium text-gray-500">Business Type</label>
                <p className="mt-1 text-gray-900">{producer.businessType}</p>
              </div>
              <div>
                <label className="text-sm font-medium text-gray-500">Business Size</label>
                <p className="mt-1 text-gray-900">{producer.businessSize}</p>
              </div>
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-6">
            ...
            <div className="mt-4 space-y-3">
              <div className="flex items-center gap-3">
                ...
                <span className="text-gray-900">{producer.user.name}</span>
              </div>
              {producer.user.email && (
                <div className="flex items-center gap-3">
              ...
              {producer.user.phone && (
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-gray-900">{producer.user.phone}</span>
                </div>
              )}
              <div className="flex items-center gap-3">
                <Calendar className="h-5 w-5 text-gray-400" />
                <span className="text-gray-900">
                  Registered on {new Date(producer.createdAt).toLocaleDateString()}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-6">
            <div className="flex items-center justify-between">
              ...
              <span className="text-sm text-gray-500">
                {producer.productCount} total products
              </span>
            </div>
            {producer.products.length === 0 ? (
              <p className="mt-4 text-gray-500">No products listed</p>
            ) : (
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-6">
            ...
            <div className="mt-4">
              <label className="flex cursor-pointer items-center gap-3 rounded-lg border border-gray-200 p-4 transition-colors hover:bg-gray-50">
                ...
                />
                <div className="flex-1">
                  <span className="font-medium text-gray-900">Verify this producer</span>
                  <p className="text-sm text-gray-500">
                    Check to approve this producer for the marketplace
                  </p>
                </div>
              </label>
            </div>
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-6">
            ...
            <div className="mt-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="font-medium text-gray-900">{producer.productCount}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-gray-600">Total Sales</span>
                <span className="font-medium text-gray-900">{producer.totalSales}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-gray-600">Rating</span>
                <span className="font-medium text-gray-900">
                  {Number(producer.ratingAverage).toFixed(1)} / 5.0
                </span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-gray-600">Featured</span>
                <span className="font-medium text-gray-900">
                  {producer.isFeatured ? "Yes" : "No"}
                </span>
              </div>
            </div>
          </div>
